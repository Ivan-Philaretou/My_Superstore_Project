...
# Define a function for creating line charts
def create_line_chart(filtered_df, x_column, y_column, title, aggregation='sum'):
    month_order = ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December']

    if aggregation == 'sum':
        y_data = filtered_df.groupby(x_column)[y_column].sum().reset_index()
        y_label = f'Total Gross Profit (in Dollars)'
        value_format = " $%{y:,.0f}"
    elif aggregation == 'sum yet not currency':
        y_data = filtered_df.groupby(x_column)[y_column].sum().reset_index()
        y_label = f'Total Quantity Ordered'
        value_format = " %{y:,.0f}"
    elif aggregation == 'count':
        y_data = filtered_df.groupby(x_column)[y_column].count().reset_index()
        y_label = f'Total Volume of Orders'
        value_format = " %{y:,.0f}"
    else:
        raise ValueError("Invalid aggregation type. Use 'sum', 'count' or 'sum yet not currency'.")

    y_data[x_column] = pd.Categorical(y_data[x_column], categories=month_order, ordered=True)
    y_data = y_data.sort_values(x_column)

    fig = px.line(y_data, x=x_column, y=y_column,
                  title=title,
    ...
    fig.update_traces(
        hovertemplate=f"<br>".join([
            f"Month: <b>%{{x}}</b>",
            f"<b><span style='text-decoration: underline; color: red;'>{y_label} per Month:</span>{value_format}</b>",
        ]),
    )
...
def filter_data(df, brand_name_option, year_option, is_discounted_option):
    if brand_name_option is not None:
        df = df[df['BRAND_NAME'] == brand_name_option]
    if year_option is not None:
        df = df[df['ORDER_YEAR'] == year_option]
    if is_discounted_option is not None:
        df = df[df['IS_DISCOUNTED'] == is_discounted_option]
    return df

def create_title(filtered_df, brand_name_option, year_option, is_discounted_option, base_title):
    if brand_name_option is not None:
    ...
    elif is_discounted_option == 1:
        is_discount_value = '(Discounted)'
    else:
        is_discount_value = ''

    title = f'{base_title} - Year: {year_value}, Brand: {brand_name_value} {is_discount_value}'
    return title

# Callbacks using the create_line_chart function
@app.callback(
    Output("output0", "children"),
    [Input('brand_name_option', 'value'), Input('year_option', 'value'), Input('is_discounted_option', 'value')]
)
def display_graph0(brand_name_option, year_option, is_discounted_option):
    filtered_df = filter_data(full_df, brand_name_option, year_option, is_discounted_option)
    title = create_title(filtered_df, brand_name_option, year_option, is_discounted_option, 'Total Order Quantity Over Time')
    return dbc.Col(dcc.Graph(figure=create_line_chart(filtered_df, 'ORDER_MONTH_NAME', 'QUANTITY', title, aggregation='sum yet not currency')))
...
def display_graph1(brand_name_option, year_option, is_discounted_option):
    filtered_df = filter_data(full_df, brand_name_option, year_option, is_discounted_option)
    title = create_title(filtered_df, brand_name_option, year_option, is_discounted_option, 'Total Gross Profit Over Time')
    return dbc.Col(dcc.Graph(figure=create_line_chart(filtered_df, 'ORDER_MONTH_NAME', 'GROSS_PROFIT_DISCOUNTED_UNIT_AMOUNT', title, aggregation='sum')))
...
def display_graph2(brand_name_option, year_option, is_discounted_option):
    filtered_df = filter_data(full_df, brand_name_option, year_option, is_discounted_option)
    title = create_title(filtered_df, brand_name_option, year_option, is_discounted_option, 'Total Volume of Orders Over Time')
    return dbc.Col(dcc.Graph(figure=create_line_chart(filtered_df, 'ORDER_MONTH_NAME', 'GROSS_PROFIT_DISCOUNTED_UNIT_AMOUNT', title, aggregation='count')))
